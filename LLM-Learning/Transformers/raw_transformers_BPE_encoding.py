"""
Trains a language model to generate Harry Potter token-by-token using transformer architecture.
"""
...
NUM_MERGES = 4000  # For BPE tokenisation
...
def get_pair_counts(word_freqs):
    """Helper function for tokenisation using pairs approach"""
    pair_counts = {}
...
def merge_pair(pair, word_freqs):
    """Another helper function for tokenisation approach"""
...
def main():
    ...
    # Split corpus using whitespace for BPE
    split_words_list = corpus_text.split()

    # The dictionary for BPE
    word_freqs = {}

    # Building tuples and counts from words for BPE tokenisation
    for word in split_words_list:
        word_tuple = tuple(char for char in word)
        if word_tuple in word_freqs.keys():
    ...
    def forward(input_ids):
        ...
        for position, token_id in enumerate(input_ids):
            final_belt.append(add_vectors(token_embeddings[token_id], position_embeddings[position]))
    ...
    def feed_forward(belt, layer):
        """Belt is the conveyor belt of tokens, layers is our learned matrices"""
        outputs = []
        for x in belt:
    ...
    def cross_entropy_loss(all_probabilities, targets):
        """Calculates the probability the model assigned the correct next token and takes
    ...
    def sample(seed_token_id):
        """Returns a string of generated text of length n_tokens"""

        context = [seed_token_id]
        generated_tokens = []

        for _ in range(SAMPLE_SIZE):
            # We run forward on the context, returning us a probability distribution for the next token
            ...
            next_token_probability_distribution = all_probabilities[-1]
            # We randomly select based on the distribution, using probabilities as our weights
            random_token_id = random.choices(range(vocab_size), weights=next_token_probability_distribution)[0]
            # And set up the next loop
            context.append(random_token_id)
            generated_tokens.append(id_to_token[random_token_id])
        # Boundary-aware join
        return "".join(" " if token == "</w>" else token for token in generated_tokens)
    ...
    # More tokenisation bumf
    corpus_token_ids = []
    cache = {}

    for word in split_words_list:
        if word not in cache:
            cache[word] = encode_word(word)
        for token in cache[word]:
            corpus_token_ids.append(token_to_id[token])
        # Adding a word-boundary separator to mitigate token soup
        corpus_token_ids.append(token_to_id["</w>"])

    corpus_length = len(corpus_token_ids)
    ...
    while True:
        ...
        # Taking our slice
        inputs = corpus_token_ids[pos : pos + SEQUENCE_LENGTH]
        targets = corpus_token_ids[pos + 1 : pos + SEQUENCE_LENGTH + 1]
